...
const int BRAKING_HOLD_TIME = 10000; 
...
unsigned long dir_change_time = 0; 
bool is_braking_sequence = false;  
...
void loop() {
  ...
  int level = inputValue / (1024 / 15.0);
  ...
  if (current_dir_switch != last_dir_switch_state) {
    if (last_dir_switch_state != -1) {
        dir_change_time = millis(); 
        is_braking_sequence = true; 
    }
  ...
  if (is_braking_sequence) {
    if (millis() - dir_change_time < BRAKING_HOLD_TIME) {
      level = 14; 
    } else {
      is_braking_sequence = false; 
    }
  }
